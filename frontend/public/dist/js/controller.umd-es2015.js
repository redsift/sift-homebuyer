 ...
 */
var HomebuyerController = (function (SiftController) {
  function HomebuyerController() {
  ...
  if ( SiftController ) HomebuyerController.__proto__ = SiftController;
  HomebuyerController.prototype = Object.create( SiftController && SiftController.prototype );
  HomebuyerController.prototype.constructor = HomebuyerController;

  // TODO: link to docs
  HomebuyerController.prototype.loadView = function loadView (state) {
    var thisMonth = moment$1().utc().startOf('month').valueOf().toString();
    var data = Promise.all([this._getIndexes(thisMonth), this._getMonthDistro(thisMonth), this._getMonthlyMean(), this._getMonthlyVolume()]).then(function (v) {
  ...
  HomebuyerController.prototype.onStorageUpdate = function onStorageUpdate (value) {
    var this$1 = this;

    var thisMonth = moment$1().utc().startOf('month').valueOf().toString();
    Promise.all([this._getIndexes(thisMonth), this._getMonthDistro(thisMonth), this._getMonthlyMean(), this._getMonthlyVolume()]).then(function (v) {
      this$1.publish('storageupdate', { month: thisMonth, indexes: v[0], monthdistro: v[1], monthlymean: v[2], monthlyvolume: v[3] });
    }).catch(function (err) {
      console.error('sift-homebuyer: onStorageUpdate: error: ', err);
    });
  };

  HomebuyerController.prototype.onLoadMonth = function onLoadMonth (value) {
    var this$1 = this;

    Promise.all([this._getIndexes(value.key), this._getMonthDistro(value.key)]).then(function (v) {
      this$1.publish('monthLoaded', { month: value.key, indexes: v[0], monthdistro: v[1] });
    });
  };

  HomebuyerController.prototype._getIndexes = function _getIndexes (month) {
  ...
  HomebuyerController.prototype._getMonthDistro = function _getMonthDistro (month) {
    return this.storage.get({
      bucket: 'monthdistro',
      keys: [month]
    }).then(function (values) {
  ...
  HomebuyerController.prototype._getMonthlyMean = function _getMonthlyMean () {
  ...
  HomebuyerController.prototype._getMonthlyVolume = function _getMonthlyVolume () {
    var mk = this._getMonthlyKeys(9);
    return this.storage.get({
  ...
  HomebuyerController.prototype._getMonthlyKeys = function _getMonthlyKeys (num) {
    var chartKeys = [];
    for (var i = 0; i < num; i += 1) {
      chartKeys.unshift(moment$1().utc().subtract(i, 'months').startOf('month').valueOf().toString());
    }
    return chartKeys;
  };

 ...
 */
function plotMonthlyMean(data, bWidth, bHeight, monthLoader) {
  ...
  var margin = { top: 20 * hscale, right: 20 * wscale, bottom: 20 * hscale, left: 28 * wscale };
  var height = 200 * hscale - margin.top - margin.bottom;
  var width = bWidth - margin.left - margin.right;
  var x = d3.scale.ordinal().rangeRoundBands([0, width], .1);
  var y = d3.scale.linear().range([height, 0]);
  x.domain(data.map(function (d) { return d.key; }));
  y.domain([0.99 * d3.min(data, function (d) { return d.value }), d3.max(data, function (d) { return d.value; })]);
  var xAxis = d3.svg.axis()
    .scale(x)
    .orient('bottom');
  xAxis.tickFormat(function (d) {
    var date = moment$1.utc(parseInt(d));
  ...
  var svg = d3.select('#monthlymean').append('svg')
  ...
  svg.append('g')
    .attr('class', 'y axis')
    .call(yAxis);
  var bar = svg.selectAll('.mabar')
    .data(data)
    .enter().append('g')
    .attr('id', function (d) { return 'mabar-' + d.key })
    .attr('class', 'mabar interactive')
    .on('click', function (d, i) {
      var bs = d3.selectAll('.mabar');
      bs.classed('selected', false);
      bs.classed('interactive', true);
      var b = d3.select('#' + bs[0][i].id);
 ...
 * Copyright (c) 2016 Redsift Limited
 */
function plotMonthDistro(values, bWidth, bHeight) {
  ...
  var margin = { top: 20 * hscale, right: 30 * wscale, bottom: 20 * hscale, left: 20 * wscale };
  var height = 180 * hscale - margin.top - margin.bottom;
  var width = bWidth - margin.left - margin.right;
  ...
  var yAxis = d3.svg.axis()
    ...
    .orient('left')
    .ticks(0);
  var svg = d3.select('#monthdistro').append('svg')
    .attr('id', 'monthdistro-svg')
    .attr('width', width + margin.left + margin.right)
    .attr('height', height + margin.top + margin.bottom)
    .append('g')
    .attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
  var bars = svg.selectAll('.mdbar')
    .data(data)
    .enter().append('g')
    .attr('class', 'mdbar')
 ...
 * Copyright (c) 2016 Redsift Limited
 */
var HomebuyerSiftView = (function (SiftView) {
  function HomebuyerSiftView() {
  ...
  if ( SiftView ) HomebuyerSiftView.__proto__ = SiftView;
  HomebuyerSiftView.prototype = Object.create( SiftView && SiftView.prototype );
  HomebuyerSiftView.prototype.constructor = HomebuyerSiftView;

  HomebuyerSiftView.prototype.presentView = function presentView (value) {
    var this$1 = this;
    ...
    plotMonthlyMean(value.data.monthlymean, block.clientWidth, block.clientHeight, function (key) {
      this$1.publish('loadMonth', { key: key });
    });
    this.plotMonthlyVolume(value.data.monthlyvolume, block.clientWidth, block.clientHeight);
    var b = d3.select('#mabar-' + value.data.month);
    b.classed('interactive', false);
    b.classed('selected', true);
  ...
  HomebuyerSiftView.prototype.willPresentView = function willPresentView (value) {};

  HomebuyerSiftView.prototype.showBlocks = function showBlocks (ids) {
  ...
  HomebuyerSiftView.prototype.updateIndexes = function updateIndexes (month, indexes) {
    if (!month) {
      ...
      document.getElementById('statssubtitle').textContent = 'on ' + moment$1.utc(parseInt(month)).format('MMM YY');
    }
  ...
  HomebuyerSiftView.prototype.onMonthLoaded = function onMonthLoaded (value) {
    this.updateIndexes(value.month, value.indexes);
    var block = document.querySelector('.block');
    plotMonthDistro(value.monthdistro, block.clientWidth, block.clientHeight);
  };

  HomebuyerSiftView.prototype.plotMonthlyVolume = function plotMonthlyVolume (data, bWidth, bHeight) {
    var wscale = 1;
    if (bWidth > 230) {
      wscale = bWidth / 230;
    }
    ...
    var margin = { left: 10 * wscale, top: 20 * hscale, right: 10 * wscale, bottom: 10 * hscale };
    var width = bWidth - margin.left - margin.right;
    var height = 220 * hscale - margin.top - margin.bottom;
    ...
    if (!this._volume) {
      ...
      this._volume = lines('volume')
        ...
        .labelTime('multi')
        .fill([
          presentation10$1.standard[presentation10$1.names.green],
          presentation10$1.standard[presentation10$1.names.red]])
        .animation('value')
        .niceIndex(false)
        .curve('curveLinear')
        .symbol(['symbolSquare', 'symbolSquare'])
        .tickDisplayIndex(function (d) {
          return moment$1.utc(d).format('MMM');
        })
        .tipHtml(function (d, i, s) {
          var tip = (s === 0) ? 'New:' : 'Reduced:';
          return tip + d[1];
        });
    }
  ...
  HomebuyerSiftView.prototype.onStorageUpdate = function onStorageUpdate (data) {
    this.presentView({ data: data });
  };

  HomebuyerSiftView.prototype.onResize = function onResize () {
    this.presentView({ data: this._currentData });
  };

  HomebuyerSiftView.prototype.onLoad = function onLoad () {
